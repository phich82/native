...
function Image_mem($file,$x,$y,$w=0,&$h,$type='',$link='')
{
	//Put an image on the page
		//get info
		if(!strlen($file))
		...
		$type=SupposeImageType($file);
		$info=false;
		if($type=="image/jpeg")
			$info=$this->_parsejpg_mem($file);
		elseif($type=="image/png")
			$info=$this->_parsepng_mem($file);
		elseif($type=="image/gif")
			$info=$this->_parsegif($file,true);
		elseif($type=="image/bmp")
			$info=$this->_parsebmp_mem($file);
		
		if(!$info)
		{
			$this->Cell($w,$this->rowheight,"Image");
				return;
...
//	read all segments
	...
	while(1)
	{
		if(strlen($file)<$ptr+4)
			break;
		$type=ord($file[$ptr+1]);
		if($type>=0xc0 && $type<=0xcf && $type!=0xc4 && $type!=0xcc)
		{
			if(strlen($file)<$ptr+10)
				break;
			$hw=unpack("nh/nw",substr($file,$ptr+5,4));
			$a[0]=$hw["w"];		// width
			$a[1]=$hw["h"]; 	// height
			$a["bits"]=ord($file[$ptr+4]);
			$a["channels"]=ord($file[$ptr+9]);
			break;
		}
		$arr=unpack("nl",substr($file,$ptr+2,2));
		$ptr+=(2+$arr["l"]);
	}
	if(!count($a))
		return false;
...
function _parsepng_mem($file)
{
	$ptr=0;
	//Check signature
	if(fread_mem($file,$ptr,8)!=chr(137).'PNG'.chr(13).chr(10).chr(26).chr(10))
		return false;
	//Read header chunk
	fread_mem($file,$ptr,4);
	if(fread_mem($file,$ptr,4)!='IHDR')
		return false;
	$w=$this->_freadint_mem($file,$ptr);
	$h=$this->_freadint_mem($file,$ptr);
	$bpc=ord(fread_mem($file,$ptr,1));
	if($bpc>8)
		return false;
	$ct=ord(fread_mem($file,$ptr,1));
	if($ct==0)
		$colspace='DeviceGray';
	elseif($ct==2)
		$colspace='DeviceRGB';
	...
	do
	{
		$n=$this->_freadint_mem($file,$ptr);
		$type=fread_mem($file,$ptr,4);
		if($type=='PLTE')
		{
			//Read palette
			$pal=fread_mem($file,$ptr,$n);
			fread_mem($file,$ptr,4);
		}
		elseif($type=='tRNS')
		{
			//Read transparency info
			$t=fread_mem($file,$ptr,$n);
			if($ct==0)
				$trns=array(ord(substr($t,1,1)));
			elseif($ct==2)
				$trns=array(ord(substr($t,1,1)),ord(substr($t,3,1)),ord(substr($t,5,1)));
		...
		{
			//Read image data block
			$data.=fread_mem($file,$ptr,$n);
			fread_mem($file,$ptr,4);
		}
		elseif($type=='IEND')
			break;
		else
			fread_mem($file,$ptr,$n+4);
	}
	while($n);
	if($colspace=='Indexed' && empty($pal))
		return false;
...
function _freadint_mem($f,&$ptr)
{
	//Read a 4-byte integer from file
	$a=unpack('Ni',fread_mem($f,$ptr,4));
	return $a['i'];
}
...
function _parsebmp_mem($f){ 
    ...
    if ($biBitCount<16){ 
        $info['cs'] = 'Indexed'; 
        $info['bpc'] = $biBitCount; 
        $palStr = fread_mem($f,$ptr, $bfOffBits-54); # palette 
        $pal = ''; 
        $cnt = strlen($palStr)/4; 
        for ($i=0;$i<$cnt;$i++){ 
            $n = 4*$i; 
    ...
    switch ($biCompression){ 
        case 0: 
            $str = fread_mem($f,$ptr, $fsize-$bfOffBits); 
            break; 
        case 1: # BI_RLE8 
            $str = $this->rle8_decode(fread_mem($f,$ptr, $fsize-$bfOffBits), $width); 
            break; 
        case 2: # BI_RLE4 
            $str = $this->rle4_decode(fread_mem($f,$ptr, $fsize-$bfOffBits), $width); 
            break; 
    } 
...
function _fread_long_le_at($f, $pos){ 
    $a=unpack('Vi', substr($f,$pos,4)); 
    return $a['i']; 
} 

function _fread_short_le_at($f, $pos){ 
    $a=unpack('vi', substr($f,$pos, 2)); 
    return $a['i']; 
} 
...
function fread_mem($file,&$ptr,$count)
{
	$ret=substr($file,$ptr,$count);
	$ptr+=$count;
	if($ptr>strlen($file))
		$ptr=strlen($file);
	return $ret;
