...
//init variables
/////////////////////////////////////////////////////////////
$query = $queryData_carsusers->Copy();
...
$templatefile = ( $inlineedit ) ? "carsusers_inline_edit.htm" : "carsusers_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: carsusers_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: carsusers_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$body["begin"]=$includes.'
	<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="carsusers_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	if($lenArr>0 and count($key) and !$_SESSION[$strTableName."_noNextPrev"])
	{
		...
		$queryData_carsusers->RemoveBlobFields(true);
		//$sql_next=gSQLWhere($where.$where_next).$order_next;
		$sql_next = $queryData_carsusers->toSql($where.$where_next, $order_next);
		//$sql_prev=gSQLWhere($where.$where_prev).$order_prev;
		$sql_prev = $queryData_carsusers->toSql($where.$where_prev, $order_prev);
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"UnblockRecord('carsusers_edit.php','".$skeys."','',function(){window.location.href='carsusers_edit.php?".$nextlink."'});return false;\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"UnblockRecord('carsusers_edit.php','".$skeys."','',function(){window.location.href='carsusers_edit.php?".$prevlink."'});return false;\"");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnblockRecord('carsusers_edit.php','".$skeys."','',function(){window.location.href='carsusers_list.php?a=return'});return false;\"");
...
else if(!in_array("id", $data)) 	
		$xt->assignbyref("id_editcontrol",$data['id']);
//	control - password
...
else if(!in_array("password", $data)) 	
		$xt->assignbyref("password_editcontrol",$data['password']);
//	control - username
...
else if(!in_array("username", $data)) 	
		$xt->assignbyref("username_editcontrol",$data['username']);
...
if(isBlockingEnable($strTableName) && $disableCtrlsForEditing=="false")
	$pageObject->AddJSCode("window.timeid".$id."=setInterval(\"ConfirmBlock('carsusers_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."')\",".($block->ConfirmTime*1000).");");
