...
//	run added control functions
		if(window["controlpool"+this.id]!=undefined)
		{
			var pool=window["controlpool"+this.id];
			for(i=0;i<pool.length;i++)
				pool[i]();
			window["controlpool"+this.id]=undefined;
		}
...
function AddScriptBe4Postload(func,id)
{	
	if(window["postloadstep"+(id ? "_"+id : "")+"_worked"])
	{
		func();
	...
	if(window["controlpool"+id]==undefined)
		window["controlpool"+id]=new Array();
	var pool = window["controlpool"+id];
	pool[pool.length]=func;
	window["controlpool"+id] = pool;	
}
...
// Unblocking record
///////////////////////////////
function UnblockRecord(page,keys,sid,func)
{
    params={
            action: 'unblock',
			keys: keys,
...
function ConfirmBlock(page,table,keys,id,inline)
{
    params={
            action: 'confirm',
			keys: keys,
    ...
    $.get(page,	params,
		function(xml)
		{
		    ...
		    {
		        if(inline=='1')
		            inlineEditing.setInputContent('block'+xml,id,'');
		        var arrCntrl = Runner.controls.ControlManager.getAt(table);
	            for(var i=0;i<arrCntrl.length;i++)
	                arrCntrl[i].setDisabled();
		        $('#system_div').html(xml);
		        $('#center_block').css('margin-top','60px');
	            $('#system_div').css('visibility','visible');
        	    $('#savebutton').attr('disabled','true');
	            $('#savebutton').css('background-color','#dcdcdc');
...
function UnblockAdmin(page,table,keys,oldid,param)
{
    params={
            action: 'unblock',
			keys: keys,
    ...
    $.get(page,	params,
		function(xml)
		{
            ...
            $('#system_div').css('visibility','hidden');
            $('#center_block').css('margin-top','0');
       	}
	);
}

function UnblockRecordInline(page,keys,id)
{
    clearInterval(window["timeid"+id]);	
    params={
            action: 'unblock',
			keys: keys,
			rndval: Math.random()
...
/**
  ...
  * str - inital text, where we do changing
  * quote_style - define mode for polishing symbols ' and "
