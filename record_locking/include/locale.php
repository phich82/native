...
$locale_info["LOCALE_ICURRENCY"]="1";
$locale_info["LOCALE_INEGCURR"]="5";
$locale_info["LOCALE_SCURRENCY"]="р.";
...
function str_format_number($val)
{
	global $locale_info;
	if(!is_numeric($val))
	  return $val;
	$val=round($val,$locale_info["LOCALE_IDIGITS"]);
	if($val>=0)
	{
	  $sign=1;
	  $int = floor($val);
