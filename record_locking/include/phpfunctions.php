...
//	move files
	...
	if($usermessage!="")
		$message = $usermessage;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==DP_INLINE || function_exists("AfterAdd") || isAuditEnable($table))
	{
 ...
 * @return array
 */
function parse_backtrace($splitAsArray = true)
{
    // get backtrace array
    ...
    {    	
		...
		{
			//$funCallsArray[] = array('path' => '['.$location.']', 'func'=>'['.$function.']', 'args'=>'['.$params.']');
...
function error_handler($errno, $errstr, $errfile, $errline)
{
	...
	$debugInfoArr = parse_backtrace();
	$xt->assign_loopsection('debugRow', $debugInfoArr);	
	
	$xt->assign('debugInfoClick', "$('#debugInfoTable').toggle(); $('#clickInfo').toggle();");
