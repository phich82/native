...
function DisplayPage(event,page,parId,control,field,tablename,category)
{
	...
	else if(page.indexOf("_fulltext.")>0)
	{
		params={
			id:id,
			rndval: Math.random()
		};
		var ifeild = field.split("&");
		for(var i=0;i<ifeild.length;i++)
		{
			var ifl = ifeild[i].split("=");
			params[""+ifl[0]+""]=ifl[1];
		}
		pagetype="fulltext";
	}
	...
	$.get(page,	params,
		function(xml)
		{
			...
			DisplayFlyDiv(xml,js,id,control,x,y-20,pagetype,field,parId);
		}
	);
	return false;
}
...
function setFlyDivDimAndCoors(paramObj, id){
	var x = parseInt(paramObj.x), y = parseInt(paramObj.y), w = parseInt(paramObj.width), h = parseInt(paramObj.height), flydiv=$("#fly"+id)[0], flycontents = $("#flycontents"+id)[0];
 ...
 * @return {object}
 */
function DisplayFlyDiv(html,js,id,control,x,y,pagetype,field,parId, oncloseHandlerCode, cfgObj)
{
	...
	if (html.length){
		var flyDinDimAndCoors = getFlyDivSizeAndCoors(id, x, y);
	}
