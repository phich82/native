...
class SearchClause
{
	 ...
	 * @var bool
	 */
	var $isUsedSrch = false;
	
	function SearchClause($tName="", $searchFieldsArr=array(), $sessionPrefix='')
	{
		global $strTableName;
		
		$this->tName = ($tName ? $tName : $strTableName);
		$this->sessionPrefix = ($sessionPrefix ? $sessionPrefix : $this->tName);
		$this->searchFieldsArr = $searchFieldsArr;
	...
	{			
		...
		if ($simpleSrch)
		{
			$sWhere = "1=0";
			for($i = 0; $i < count($this->searchFieldsArr); $i++) {
				if($where = StrWhereExpression($this->searchFieldsArr[$i], $simpleSrch, "Contains", ""))
					$sWhere.= " or ".$where;
			}	
		...
		// if there fields for build advanced where
		$sWhere = '';
		if (count($srchFields)){
			// prepare vars
			...
			{								
				if($where=StrWhereAdv($srchF['fName'], $srchF['value1'], $srchF['opt'], $srchF['value2'], $srchF['eType']))
				{
					// add not 
					if($srchF['not'])
						$where="not (".$where.")";
					// and use only if different field names	
					if($srchType=="and" && $prevSrchFieldName != $srchF['fName'])
	...
	{		
		...
		switch ($this->srchType){
			...
			case 'simple' :
				...
				break;
			case 'integrated' :
				$sWhere = $this->buildItegratedWhere();	
				break;
			default:
				$sWhere = '';				
		}
	...
	function getSrchPanelAttrs(){
		return array('srchOptShowStatus' => $this->_where[$this->sessionPrefix."_srchOptShowStatus"],
