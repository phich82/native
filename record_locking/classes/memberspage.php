...
{	
	...
	{	
		...
		$this->xt->assign("savebutton_attrs","onclick=\"document.forms.frmAdmin".$this->id.".a.value = 'save'; document.forms.frmAdmin".$this->id.".submit();\"");
		$this->xt->assign("resetbutton_attrs","onclick=\"document.forms.frmAdmin".$this->id.".reset();\"");		
		...
		if($this->sortByGroup !== false || @$_SESSION[$this->sessionPrefix."_orderby"]!="A")
	...
	{							
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize))
		{
			...
			$row["begin"]="<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["username"])."\">";
			$row["username"]=htmlspecialchars($data["username"]);
			$row["user"]=$data["username"];
			$usercheckbox_attrs.=" name=\"user_".htmlspecialchars($data["username"])."\"";
			$usercheckbox_attrs.=" onclick=\"var ch=document.getElementsByName('cb_".$this->recNo."[]');
	...
	function addCommonJS() 
	{
		...
		// additional js
		$this->AddJSCode("
			window.INLINE_EDIT_TABLE='".$this->tName."_edit.php';
			window.INLINE_ADD_TABLE='".$this->tName."_add.php';
			window.INLINE_VIEW_TABLE='".$this->tName."_view.php';
			window.SUGGEST_TABLE='".$this->tName."_searchsuggest.php';
			window.MASTER_PREVIEW_TABLE='".$this->tName."_masterpreview.php';
		");
	}
	...
	function setSessionVariables() 
	{
		...
		if(!@$_SESSION[$this->sessionPrefix."_orderby"] || $this->userOrdeBy == "A" || $this->userOrdeBy == "D")
		{
	...
	{		
		...
		// apply pagination
			...
			if(count($rowInfo)>$this->pageSize)
				array_splice($rowInfo,$this->pageSize,count($rowInfo)-$this->pageSize);
			$this->recNo=1;
			$shade=false;
			for($i=0;$i<count($rowInfo);$i++)
			{
				...
				if(!$shade)
				{
					$rowInfo[$i]["rowattrs"]="class=shade onmouseover=\"this.className = 'rowselected';\" onmouseout=\"this.className = 'shade';\"";
					$shade=true;
				}
				else
				{
					$rowInfo[$i]["rowattrs"]="onmouseover=\"this.className = 'rowselected';\" onmouseout=\"this.className = '';\"";
					$shade=false;
				}
			}
		}
	...
	{
		if($this->userOrdeBy == "D")
		{
			$this->strOrderBy = "Order by 1 desc";
		}else{
			$this->strOrderBy = "Order by 1 asc";
		}
	}	
