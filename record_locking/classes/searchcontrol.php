...
class SearchControl {
	...
	var $pageObj = null;
	var $searchClause = false;
	var $id = 1;
	
	
	function SearchControl($id, $tName='', &$searchClause, &$pageObj)
	{
		$this->tName = $tName;
		
		$this->searchClause = $searchClause;		
		$this->getSrchPanelAttrs = $this->searchClause->getSrchPanelAttrs();
		$this->globSrchParams = $this->searchClause->getSearchGlobalParams();
	...
	function isNeedSecondCtrl($fName)
	{
		...
		if ($fType == EDIT_FORMAT_DATE || $fType == EDIT_FORMAT_TEXT_FIELD || $fType == EDIT_FORMAT_TEXT_AREA
	...
	function getCtrlSearchTypeOptions($fName, $selOpt) 
	{
		...
		{
			$options.="<OPTION VALUE=\"Equals\" ".(($selOpt=="Equals")?"selected":"").">"."Equals"."</option>";
			$options.="<OPTION VALUE=\"More than\" ".(($selOpt=="More than")?"selected":"").">"."More than ..."."</option>";
			$options.="<OPTION VALUE=\"Less than\" ".(($selOpt=="Less than")?"selected":"").">"."Less than ..."."</option>";
			$options.="<OPTION VALUE=\"Between\" ".(($selOpt=="Between")?"selected":"").">"."Between"."</option>";
			$options.="<OPTION VALUE=\"Empty\" ".(($selOpt=="Empty")?"selected":"").">"."Empty"."</option>";
		}
		elseif ($fType == EDIT_FORMAT_LOOKUP_WIZARD)
		{
			if (Multiselect($fName, $this->tName)){
				$options.="<OPTION VALUE=\"Contains\" ".(($selOpt=="Contains")?"selected":"").">"."Contains"."</option>";	
			}else{
				$options.="<OPTION VALUE=\"Equals\" ".(($selOpt=="Equals")?"selected":"").">"."Equals"."</option>";	
			}
		}
					...
					|| $fType == EDIT_FORMAT_HIDDEN || $fType == EDIT_FORMAT_READONLY)
		{
			$options.="<OPTION VALUE=\"Contains\" ".(($selOpt=="Contains")?"selected":"").">"."Contains"."</option>";
			$options.="<OPTION VALUE=\"Equals\" ".(($selOpt=="Equals")?"selected":"").">"."Equals"."</option>";
			$options.="<OPTION VALUE=\"Starts with\" ".(($selOpt=="Starts with")?"selected":"").">"."Starts with ..."."</option>";
			$options.="<OPTION VALUE=\"More than\" ".(($selOpt=="More than")?"selected":"").">"."More than ..."."</option>";
			$options.="<OPTION VALUE=\"Less than\" ".(($selOpt=="Less than")?"selected":"").">"."Less than ..."."</option>";
			$options.="<OPTION VALUE=\"Between\" ".(($selOpt=="Between")?"selected":"").">"."Between"."</option>";
			$options.="<OPTION VALUE=\"Empty\" ".(($selOpt=="Empty")?"selected":"").">"."Empty"."</option>";
	...
	function getCtrlSearchType($fName, $recId, $fieldNum=0, $selOpt, $renderHidden=false) 
	{	
		...
		$searchtype .= $this->getCtrlSearchTypeOptions($fName, $selOpt);
		$searchtype .= "</SELECT>";
	...
	function buildSearchCtrlBlockArr($recId, $fName, $ctrlInd, $opt, $not, $isChached, $val1, $val2)
	{
		...
		// combo with attrs
		$srchCtrlBlock['searchtype'] = $this->getCtrlSearchType($fName, $recId, $ctrlInd, $opt);		
	...
	{		
		$categoryFieldParams = $this->searchClause->getSearchCtrlParams($this->getParentCtrlName($fName));
	...
	{
		return "onkeydown=\"return listenEvent(event, this, searchController".$this->id.");\" onkeyup=\"searchSuggest(event,this,'ordinary','".GetTableURL($this->tName)."_searchsuggest.php');\"";
	}
