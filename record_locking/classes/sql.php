...
// Data from wizard
///////////////////////////////////////////////////////////////////////////
class SQLNonParsedBase
{
	var $m_sql;
	function SQLNonParsedBase($proto)
	{
		$m_sql = isset($proto["m_sql"]) ? $proto["m_sql"] : '';
	}
}
...
class SQLFieldListItemBase
{
	var $m_expr;
	var $m_alias;
      
	function SQLFieldListItemBase($proto)
	{
...
class SQLFromListItemBase
{
	...
	function SQLFromListItemBase($proto)
	{
...
class SQLFunctionCallBase
{
	var $m_functiontype;
	var $m_strFunctionName;
	var $m_arguments;
      
	function SQLFunctionCallBase($proto)
	{
...
class SQLGroupByItemBase
{
	var $m_column;
      
	function SQLGroupByItemBase($proto)
	{
...
class SQLLogicalExprBase
{
	...
	function SQLLogicalExprBase($proto)
	{
...
class SQLOrderByItemBase
{
	var $m_column;
	var $m_bAsc;
	var $m_nColumn;
     
	function SQLOrderByItemBase($proto)
	{
		$this->m_column = isset($proto["m_column"]) ? $proto["m_column"] : null;
...
// Implementation
///////////////////////////////////////////////////////////////////////////
class SQLNonParsed extends SQLNonParsedBase
{
	function SQLNonParsed($proto)
	{
		SQLNonParsedBase::SQLNonParsedBase($proto);
	}
...
class SQLField extends SQLFieldBase
{
	function SQLField($proto)
	{
		SQLFieldBase::SQLFieldBase($proto);
	}
	...
	function toSql($query)
	{
		...
		{
			return AddFieldWrappers($this->m_strName);
		}
		else
		{
			return AddFieldWrappers($this->m_strTable) . "." . AddFieldWrappers($this->m_strName);
		}
	}
...
class SQLFieldListItem extends SQLFieldListItemBase
{
	function SQLFieldListItem($proto)
	{
		SQLFieldListItemBase::SQLFieldListItemBase($proto);
	}
...
class SQLFromListItem extends SQLFromListItemBase
{
	function SQLFromListItem($proto)
	{
		SQLFromListItemBase::SQLFromListItemBase($proto);
	}
	...
	function toSql($query, $first)
	{
		...
		if($this->m_alias != '')
		{
			$ret .= ' as ' . AddFieldWrappers(m_alias);
		}
...
class SQLFunctionCall extends SQLFunctionCallBase
{
	function SQLFunctionCall($proto)
	{
		SQLFunctionCallBase::SQLFunctionCallBase($proto);
	}
...
class SQLGroupByItem extends SQLGroupByItemBase
{
	function SQLGroupByItem($proto)
	{
		SQLGroupByItemBase::SQLGroupByItemBase($proto);
	}
...
class SQLLogicalExpr extends SQLLogicalExprBase
{
	var $query;
	var $postSql;
	...
	{
		SQLLogicalExprBase::SQLLogicalExprBase($proto);
		$this->postSql = array();
	}
	...
	function AddFilter($field, $func, $value = '')
	{
		if(is_string($field))
		{
			$field = $this->query->DefaultTable()->FieldByName($field);
		}
	...
	{
		$tableKeys = $this->query->DefaultTable()->GetKeyFields();
		foreach($tableKeys as $tk)
		{
			$value = make_db_value($tk, $keys[$tk]);
			if(SQLQuery::IsNull($value))
			{
...
class SQLOrderByItem extends SQLOrderByItemBase
{
	function SQLOrderByItem($proto)
	{
		SQLOrderByItemBase::SQLOrderByItemBase($proto);
	}
...
class SQLTable extends SQLTableBase
{
	var $query;
	...
	{
		SQLTableBase::SQLTableBase($proto);
	}
	
	function toSql($query)
	{
		return AddFieldWrappers($this->m_strName);//SmartAddTableWrappers(m_strName);
	}
	
	function GetKeyFields()
	{
		return GetKeyFields($this->m_strName);
	}
...
class SQLQuery extends SQLQueryBase
{
	var $subqueriesSupportedGlobal = true;
	
	function SQLQuery($proto)
	{
		SQLQueryBase::SQLQueryBase($proto);
		...
		$this->subqueriesSupportedGlobal = $bSubqueriesSupported;
	}
	...
	function toSql($strwhere = null, $strorderby = null)
	{
		$sql = $this->HeadToSql();
		
		// allow derived classes to drop out fields from field list
		if(count($this->m_fieldlist) > 0)
		{
			$sql .= "\r\n";
			$sql .= ' FROM ';
		}
		
		if(1 /* dbtype != MS_ACCESS || */)
		{
			$fromlist = array();
			for($nCnt = 0; $nCnt < count($this->m_fromlist); $nCnt ++)
			{
	...
	function RemoveBlobFields($useDummies = true)
	{
		$newFields = array();
		for($nCnt = 0; $nCnt < count($this->m_fieldlist); $nCnt ++)
		{
			$field = $this->m_fieldlist[$nCnt]->m_expr;
			if(is_object($field) && $field->IsBinary())
			{
				if($useDummies)
				{
					$fli = new SQLFieldListItem(array(
						"m_alias" => "runnerdummy" . $nCnt,
						"m_expr" => "1",
					));
					$newFields []= $fli;
				}
				...
				{
					$this->dropOrderByField($field);
				}
			}
			else
			{
				$newFields []= $this->m_fieldlist[$nCnt];
			}
		}
		$this->m_fieldlist = $newFields;
	}
	
	function RemoveAllFieldsExcept($except_name, $except_table)
	{
		$newFields = array();
		for($nCnt = 0; $nCnt < count($this->m_fieldlist); $nCnt ++)
		{
			$field = $this->m_fieldlist[$nCnt]->m_expr;
			if(is_object($field) && ($field->m_strTable != $except_table || $field->m_strName != $except_name))
			{
				$fli = new SQLFieldListItem(array(
					"m_alias" => "runnerdummy" . $nCnt,
					"m_expr" => "1",
