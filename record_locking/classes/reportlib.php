...
function create_reportfield($name, $type, $interval, $alias)
{
	...
	if($type)
	{
		...
		{
			return new $report_classfields[$type]($name, $interval, $alias);
		}
		else
		{
...
class ReportField
{
	...
	function ReportField($name, $interval, $alias)
	{
		$this->_name = $name;
	...
	{
        return $this->getStringSql() . ($this->alias() ? ' as ' . cached_ffn($this->alias()) : '');
	}
...
{
	function ReportNumericField($name, $interval, $alias)
	{
		ReportField::ReportField($name, $interval, $alias);
	}
...
{
	function ReportCharField($name, $interval, $alias)
	{
		ReportField::ReportField($name, $interval, $alias);
	}
	...
	function getWhereSql($groups)
	{
		...
		if(count($groups) > 0)
		{
			...
			$ret = '('.$ssql.$gt.'\''.db_addslashes($groups[0]).'\' AND '.$ssql.$lt.'\''.db_addslashes($groups[count($groups) - 1]).'\')';
		}
...
{
	function ReportDateField($name, $interval, $alias)
	{
		ReportField::ReportField($name, $interval, $alias);
	}
	...
	function getFieldName($fieldValue, $data)
	{
		...
		if($this->_interval == 0)
		{
			if($this->_viewFormat)
			{
				return GetDataValue($value, $this->_name, $this->_viewFormat);
			}
			else
			{
				$date = db2time($value);
...
function getFormattedValue($value, $fieldName, $strViewFormat, $strEditFormat = '')
{
	...
	else
	{
		if($strEditFormat == EDIT_FORMAT_LOOKUP_WIZARD || $strEditFormat == EDIT_FORMAT_RADIO)
			$val = DisplayLookupWizard($fieldName, $value, null, '', MODE_LIST);
		else
			$val = GetDataValue($value, $fieldName, $strViewFormat);
	}
...
class SQLStatement
{
	...
	function SQLStatement($sql, $order, $pagesize, $connection, &$searchClause, &$params)
	{
        ...
        for($i=0; $i<count($this->repGroupFields); $i++)
        {
	        for($j=0; $j<count($this->fieldsArr); $j++)
	        {
		        if ($this->repGroupFields[$i]['strGroupField'] == $this->fieldsArr[$j]['name'])
		        {
					...
					if ($this->fieldsArr[$j]['totalType'] == TOTAL_MAX 
						|| $this->fieldsArr[$j]['totalType'] == TOTAL_MIN
						|| $this->fieldsArr[$j]['totalType'] == TOTAL_SUM
						|| $this->fieldsArr[$j]['totalType'] == TOTAL_AVG)
					{
						$add['rowsinsummary'] ++;
					}
        ...
        {	
			if ($this->fieldsArr[$i]['totalType'] == TOTAL_MAX)
			{
				$this->_aggregates []= 'MAX('.cached_ffn($this->fieldsArr[$i]['name']).') as '.AddFieldWrappers(GoodFieldName($this->fieldsArr[$i]['name']."MAX"));
            }else if ($this->fieldsArr[$i]['totalType'] == TOTAL_MIN){		
				$this->_aggregates []= 'MIN('.cached_ffn($this->fieldsArr[$i]['name']).') as '.AddFieldWrappers(GoodFieldName($this->fieldsArr[$i]['name']."MIN"));
            }else if ($this->fieldsArr[$i]['totalType'] == TOTAL_AVG){	
	            if (!IsDateFieldType(GetFieldType($this->fieldsArr[$i]['name'], $this->tName)))
	            {  
					$this->_aggregates []= 'AVG('.cached_ffn($this->fieldsArr[$i]['name']).') as '.AddFieldWrappers(GoodFieldName($this->fieldsArr[$i]['name']."AVG"));
					$this->_aggregates []= 'COUNT('.cached_ffn($this->fieldsArr[$i]['name']).') as '.AddFieldWrappers(GoodFieldName($this->fieldsArr[$i]['name']."NAVG"));
	            }
			}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_SUM){
				if (!IsDateFieldType(GetFieldType($this->fieldsArr[$i]['name'], $this->tName)))
	            {
	          		$this->_aggregates []= 'SUM('.cached_ffn($this->fieldsArr[$i]['name']).') as '.AddFieldWrappers(GoodFieldName($this->fieldsArr[$i]['name']."SUM"));
	            }
			}
        }
	...
	function sqlcg()
	{
		...
		return 'select count(*) as c from ('.$gsql.') countgroups';
	}
	...
	{
		return $this->searchClause->applyWhere($sql);
	}
...
class Summarable
{
	...
	function addSummary($recordsMode, &$summary, $data, &$nTotalRecords)
	{
		...
		if($recordsMode)
		{
			for($i=0; $i<count($this->fieldsArr); $i++)
			{
		   		if ($this->fieldsArr[$i]['totalType'] == TOTAL_NONE)
		   		{
		   			continue;
		   		}
				if($data[$this->fieldsArr[$i]['name']] !== null)
				{
					...
					if ($this->fieldsArr[$i]['totalType'] == TOTAL_MAX)
					{
						...
						$s[$this->fieldsArr[$i]['name']]['AVG'] /= $s[$this->fieldsArr[$i]['name']]['count'];
					}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_SUM){
						if ($this->fieldsArr[$i]['viewFormat'] == "Time")
						{						
...
class ReportLogic extends Summarable
{
    ...
    function ReportLogic($sql, $order, &$searchClause, $connection, $pagesize, $printpagesize, &$params)
    {
    	RunnerApply($this, $params);
		Summarable::Summarable($params);
        $this->_connection = $connection;
        $this->searchClause = $searchClause;
		$this->_sql = new SQLStatement($sql, $order, $pagesize, $connection, $searchClause, $params);
	...
	function & appendRow($row)
	{
		if($this->_doPaging)
		{
			...
			if($page > 0 && !isset($this->_list[$page - 1]))
				die("Please, increase numer of records per print page to display report properly");
			$this->_list[$page][] = $row;
	...
	function getReport($from = 0)
	{
		...
		if(!$this->_recordBasedRequest)
		{
			...
			$sql = $this->_sql->sql2($groups);
		    $cursor = db_query($sql, $this->_connection);
		    while($data = db_fetch_array($cursor))
		    {
				$this->setSummary($this->_sql->_details, $data,
		...
		else
		{
	        ...
	        while($data = db_fetch_array($cursor))
			{
				...
				if ($this->repGroupFieldsCount)
				{
					$visible = $this->_doPaging || $this->_groups->isVisibleGroup();
				}else{
		...
		return array('list' => $this->_list, 'global' => $globals, 'page' => $this->_summary,
					'maxpages' => $maxpages, 'countRows'=>$nRow, 'countGroups'=>$countGroups);
	}
}
...
class Report extends ReportLogic
{
    ...
    function Report($sql, $order, &$searchClause, $connection, $pagesize, $printpagesize, &$params)
    {
    	// copy properties to object
		RunnerApply($this, $params);	
		
		ReportLogic::ReportLogic($sql, $order, $searchClause, $connection, $pagesize, $printpagesize, $params);
	}
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				if(!$this->forExport)
				{
					...
					{
						$val.="<a target=_blank href=\"".$this->shortTName."_imager.php?field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
						$val.= "<img border=0";
						if(isEnableSection508())
							$val.= " alt=\"Image from DB\"";
						$val.=" src=\"".$this->shortTName."_imager.php?field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['thumbnail']))."&alt=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['repPage'])).$keylink."\">";
						$val.= "</a>";
					}else{
						$val = "<img";
						...
						if(isEnableSection508())
							$val.= " alt=\"Image from DB\"";
						$val.=" src=\"".$this->shortTName."_imager.php?field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['name'])).$keylink."\">";
					}
				}
				else
				{
				...
				if(!$this->forExport)
				{
					if(CheckImageExtension($value[$this->fieldsArr[$i]['name']])) 
					{
						if ($this->fieldsArr[$i]['showThumb'])
						{
							...
							if(substr($this->fieldsArr[$i]['strhlPrefix'],0,7)!="http://" && !myfile_exists(GetUploadFolder($this->fieldsArr[$i]['name']).$thumbname))
								$thumbname = $value[$this->fieldsArr[$i]['name']];
							$val = "<a target=_blank href=\"".htmlspecialchars(AddLinkPrefix($this->fieldsArr[$i]['name'], $value[$this->fieldsArr[$i]['name']]))."\">";
							$val .= "<img";
							...
							$val .= " border=0";
							if(isEnableSection508())
								$val.= " alt=\"".htmlspecialchars($data[$this->fieldsArr[$i]['name']])."\"";
				...
				if(!$this->forExport)
				{
					...
					{
						$val = "<a href=\"".$this->shortTName."_getfile.php?filename=".rawurlencode($filename)."&field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['repPage'])).$keylink."\">";
			...
			}elseif(($this->fieldsArr[$i]['editFormat'] == EDIT_FORMAT_LOOKUP_WIZARD || $this->fieldsArr[$i]['editFormat'] == EDIT_FORMAT_RADIO) && pLookupObj.nLookupType == LT_LOOKUPTABLE){			
				$val = DisplayLookupWizard($this->fieldsArr[$i]['name'], $value[$this->fieldsArr[$i]['name']], $value, $keylink, MODE_LIST);
			}elseif(NeedEncode($this->fieldsArr[$i]['name'], $this->tName)){
				$val = ProcessLargeText(GetData($value, $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat']), "field=".rawurlencode(htmlspecialchars($this->fieldsArr[$i]['repPage'])).$keylink, "", MODE_LIST);
			}else{
				...
				for($j=0; $j<count($this->fieldsArr); $j++)
				{
					if(is_array($grp['summary']))
					{
						if(is_array($grp['summary'][$this->fieldsArr[$j]['name']]))
						{
							if ($this->fieldsArr[$j]['totalType'] == TOTAL_MAX)
							{
								$end["group".$this->fieldsArr[$j]['strGroupField']."_total".$this->fieldsArr[$j]['goodName']."_max"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['MAX'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat']);
							}else if ($this->fieldsArr[$j]['totalType'] == TOTAL_MIN){							
								$end["group".$this->fieldsArr[$j]['strGroupField']."_total".$this->fieldsArr[$j]['goodName']."_min"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['MIN'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat']);
							}else if ($this->fieldsArr[$j]['totalType'] == TOTAL_AVG){							
								$end["group".$this->fieldsArr[$j]['strGroupField']."_total".$this->fieldsArr[$j]['goodName']."_avg"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['AVG'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat']);
							}else if ($this->fieldsArr[$j]['totalType'] == TOTAL_SUM){		
								$end["group".$this->fieldsArr[$j]['strGroupField']."_total".$this->fieldsArr[$j]['goodName']."_sum"] =
									getFormattedValue($grp['summary'][$this->fieldsArr[$j]['name']]['SUM'], $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat']);
							}
						}
					}
					if ($this->fieldsArr[$j]['name'] == $this->repGroupFields[$i]['strGroupField'])
					{
						...
						{
							$begin[GoodFieldName($gname.'_grval')] = $gvalue;
							if ($this->showGroupSummaryCount)
							{
								$end[GoodFieldName($gname.'_grval')] = $gvalue;
							}
						}
						else
						{
							$formattedValue = getFormattedValue($gvalue, $this->fieldsArr[$j]['name'], $this->fieldsArr[$j]['viewFormat'], $this->fieldsArr[$j]['editFormat']);
							$begin[GoodFieldName($gname.'_grval')] = $formattedValue;
							if ($this->showGroupSummaryCount)
							{
								$end[GoodFieldName($gname.'_grval')] = $formattedValue;
							}
						}
					}
				}                        
	...
	function _writePage(&$page, $src, $count)
	{
		...
		if ($this->repPageSummary)
		{
	        for($i=0; $i<count($this->fieldsArr); $i++)
	        {
				...
				{
					if ($this->fieldsArr[$i]['totalType'] == TOTAL_SUM)
					{
						$page["page_total".$this->fieldsArr[$i]['goodName']."_sum"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['SUM'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
					}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_AVG){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_avg"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['AVG'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
					}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_MIN){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_min"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['MIN'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
					}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_MAX){	
						$page["page_total".$this->fieldsArr[$i]['goodName']."_max"] = getFormattedValue($src[$this->fieldsArr[$i]['name']]['MAX'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
					}
				}
			}
			...
			if(is_array($source["summary"]))
			{
				for($i=0; $i<count($this->fieldsArr); $i++)
				{
					if(is_array($source["summary"][$this->fieldsArr[$i]['name']]))
					{
						if ($this->fieldsArr[$i]['totalType'] == TOTAL_MAX)
						{
						    $result["global_total".$this->fieldsArr[$i]['goodName']."_max"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['MAX'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
						}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_MIN){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_min"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['MIN'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
						}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_AVG){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_avg"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['AVG'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
						}else if ($this->fieldsArr[$i]['totalType'] == TOTAL_SUM){	
							$result["global_total".$this->fieldsArr[$i]['goodName']."_sum"] = getFormattedValue($source['summary'][$this->fieldsArr[$i]['name']]['SUM'], $this->fieldsArr[$i]['name'], $this->fieldsArr[$i]['viewFormat'], $this->fieldsArr[$i]['editFormat']);
						}
					}
				}
			}
