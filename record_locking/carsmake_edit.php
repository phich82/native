...
$templatefile = ( $inlineedit ) ? "carsmake_inline_edit.htm" : "carsmake_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: carsmake_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: carsmake_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$body["begin"]=$includes.'
	<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="carsmake_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	if($lenArr>0 and count($key) and !$_SESSION[$strTableName."_noNextPrev"])
	{
		...
		$queryData_carsmake->RemoveBlobFields(true);
		//$sql_next=gSQLWhere($where.$where_next).$order_next;
		$sql_next = $queryData_carsmake->toSql($where.$where_next, $order_next);
		//$sql_prev=gSQLWhere($where.$where_prev).$order_prev;
		$sql_prev = $queryData_carsmake->toSql($where.$where_prev, $order_prev);
	...
	if(count($next))
	{
		...
		$xt->assign("nextbutton_attrs","align=\"absmiddle\" onclick=\"UnblockRecord('carsmake_edit.php','".$skeys."','',function(){window.location.href='carsmake_edit.php?".$nextlink."'});return false;\"");
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","align=\"absmiddle\" onclick=\"UnblockRecord('carsmake_edit.php','".$skeys."','',function(){window.location.href='carsmake_edit.php?".$prevlink."'});return false;\"");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnblockRecord('carsmake_edit.php','".$skeys."','',function(){window.location.href='carsmake_list.php?a=return'});return false;\"");
...
if(isBlockingEnable($strTableName) && $disableCtrlsForEditing=="false")
	$pageObject->AddJSCode("window.timeid".$id."=setInterval(\"ConfirmBlock('carsmake_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."')\",".($block->ConfirmTime*1000).");");
